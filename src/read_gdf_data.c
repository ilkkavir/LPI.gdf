...
SEXP read_gdf_data_R( SEXP ndata , SEXP nfiles , SEXP filepaths, SEXP istart , SEXP iend , SEXP bigendian , SEXP controlbits)
{
  ...
  // The actual reading
  s = read_gdf_data( cdata , idatar , idatai , ndata , nfiles, filepaths, istart , iend , bigendian , controlbits );
 ...
 */
SEXP read_gdf_data( SEXP cdata , SEXP idatar , SEXP idatai , SEXP ndata , SEXP nfiles , SEXP filepaths , SEXP istart , SEXP iend , SEXP bigendian , SEXP controlbits )
{
  ...
  for( k=0 ; k<(*nf) ; ++k ) {
    ...
    *isuccess = *isuccess && (rr==(ie[k]-is[k]+1)*4);
	...
	ii = ( rblock[kd*4+3] << 8 ) + rblock[kd*4+2];
      ...
      if( cb ){ // lowest bits are control bits
